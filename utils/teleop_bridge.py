...
Reads retargeted upper-body joint angles and hand commands from Redis
...
#          [18:21] waist      → our WAIST_JOINTS [12,13,14]
...
def extract_upper_body_from_mimic_obs(mimic_obs):
    """Extract waist + arm joint angles from TWIST2's 35D mimic observation.

    Returns dict {unitree_joint_idx: angle_rad} for the 17 upper-body joints.
    """
...
class RedisReader:
    ...
    def read(self) -> TeleopFrame:
        ...
        frame.upper_body = extract_upper_body_from_mimic_obs(mimic_obs)
...
class RobotSender:
    ...
    def __init__(self, network=None):
        ...
        print("DDS initialized, waiting for robot state...")
    ...
    def get_current_positions(self):
        with self._lock:
            if self.low_state is None:
                return {j: 0.0 for j in ARM_SDK_JOINTS}
            return {
...
class MockRobotSender:
    ...
    def __init__(self):
        self._positions = {j: 0.0 for j in ARM_SDK_JOINTS}
        self._step = 0
        print("MockRobotSender: commands will be logged (no real robot)")
    ...
    def send_arm(self, positions):
        ...
        if self._step % (CONTROL_HZ * 2) == 0:
            ...
            print(
                ...
                f"waist={[f'{v:.2f}' for v in waist]}  "
                f"L_arm={[f'{v:.2f}' for v in larm]}  "
...
class TeleopBridge:
    ...
    def __init__(self, reader, sender, recorder=None):
        self.reader = reader
        self.sender = sender
...
def main():
    ...
    print("  TWIST2 (PICO VR) → Upper Body Control")
    print("=" * 55)

    if args.mock:
        reader = MockRedisReader(motion=args.mock_motion)
        sender = MockRobotSender()
    else:
        reader = RedisReader(host=args.redis_ip, port=args.redis_port)
        sender = RobotSender(network=args.network)
